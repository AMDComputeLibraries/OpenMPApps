...
if test -d /bgsys; then  # running on BG/Q
  ...
  if test -d /gpfs; then # TODO: change this when Mira comes online
      ...
      echo "qsub  -node $3  --mode c$CORECOUNT -A $PROJECT -t $REQ_WALLTIME -O $OUTFILE nekbone $SESSION"
      COBALTJOB=`qsub -n $3  --mode c$CORECOUNT -A $PROJECT -t $REQ_WALLTIME -O $OUTFILE nekbone $SESSION`
      ...
      echo "cqsub  -n $CORECOUNT -m vn -p $PROJECT -q $PROSIZE -e BG_MAPPING=TXYZ -t $REQ_WALLTIME -O $OUTFILE nekbone $SESSION"
      COBALTJOB=`cqsub -n $CORECOUNT -m vn -p $PROJECT -q $PROSIZE -e BG_MAPPING=TXYZ -t $REQ_WALLTIME -O $OUTFILE nekbone $SESSION`
  ...
  if test "$WAIT" = "1"; then
    ...
    echo "aprun -n $CORECOUNT -d 2 -cc depth -j 1 ./nekbone $SESSION" >> $QSUBSCRIPT
    ...
    echo " aprun -n $CORECOUNT -N 1 ./nekbone ">> $QSUBSCRIPT
    ...
    echo "jsrun -n$CORECOUNT -a1 -g1 ./nekbone $SESSION" >> $BSUBSCRIPT
    ...
    mpiexec -np $2 ./nekbone $SESSION > $SESSION.np=$2.output
fi
