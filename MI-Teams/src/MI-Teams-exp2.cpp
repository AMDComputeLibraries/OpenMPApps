...
// initialize on host
  ...
  #pragma omp target data map(alloc: g[:MB*E], h[:M], lengths[:MB], indices[:PT]) map(from: W[:M*E])
  {
   ...
   #pragma omp target teams distribute parallel for num_teams(NUMTEAMS) num_threads(NUMTHREADS) //thread_limit(len)
   {
     for (uint64_t sample=0; sample <MB; sample++) {
	   ...
	   W[sample*len + i*E+e] += g[sample*E+e] * float_step; //# update weights
       }
     }
   } // end of 2nd kernel
