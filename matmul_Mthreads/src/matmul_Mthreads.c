...
#pragma omp parallel for
  ...
  for (int dev=0; dev < DevS; dev++) {
    #pragma omp target enter data device(0) map(to: matA, matB, matC)
  }
...
#pragma omp parallel for num_threads(DevS)
  for (int dev=0; dev < DevS; dev++) {
    ...
    #pragma omp target teams device(0)//num_teams(numteams) //thread_limit(numthreads)
  ...
  for (int dev=0; dev <DevS; dev++) {
  #pragma omp target exit data device(0) map(from: matC[dev*N/DevS:N/DevS])
  }
